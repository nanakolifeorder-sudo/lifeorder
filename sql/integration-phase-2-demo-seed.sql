...
select p.tenant_slug, p.code, 'A', seed.dimension_key, seed.name, seed.description, 10, 'bar', seed.sort_order, '啟用'
  from projects p
 cross join (values
...
select q.tenant_slug, q.id, seed.option_key, seed.label, seed.score_weights::jsonb, seed.sort_order, '啟用'
  from q
  join (values
    (1, 'A', '我還不太確定，只是想先了解', '{"clarity":1,"urgency":1}', 10),
    (1, 'B', '我有明確問題，但還不知道怎麼處理', '{"clarity":3,"urgency":2}', 20),
    (1, 'C', '我已經想處理，正在找適合方案', '{"clarity":5,"urgency":4}', 30),
    (2, 'A', '剛開始出現', '{"clarity":1,"urgency":1}', 10),
    (2, 'B', '持續一段時間了', '{"clarity":2,"urgency":3}', 20),
    (2, 'C', '已經影響到我的安排或決策', '{"clarity":3,"urgency":5}', 30),
    (3, 'A', '釐清問題', '{"clarity":2,"urgency":1}', 10),
    (3, 'B', '知道下一步怎麼做', '{"clarity":3,"urgency":2}', 20),
    (3, 'C', '盡快安排實作或服務', '{"clarity":2,"urgency":4}', 30)
