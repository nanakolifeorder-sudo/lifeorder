...
select p.tenant_slug, p.code, 'LITE', 'A 版測驗', '整合版測驗漏斗 MVP 範例版本', '啟用'
  from projects p
 where p.code = 'LO'
...
select p.tenant_slug, p.code, 'LITE', seed.stage_key, seed.title, seed.description, seed.sort_order, '啟用'
  from projects p
 cross join (values
...
select p.tenant_slug, p.code, 'LITE', seed.dimension_key, seed.name, seed.description, 10, 100, 'number', 'round', 'radar', seed.sort_order, '啟用'
  from projects p
 cross join (values
...
select p.tenant_slug, p.code, 'LITE', seed.dimension_key, seed.range_key, seed.label, seed.min_score, seed.max_score, seed.title, seed.subtitle, seed.body, seed.sort_order, '啟用'
  from projects p
 cross join (values
...
select p.tenant_slug, p.code, 'LITE', 'score_ranges', '你的檢測報告', '以下內容會依照你的作答與分數區間自動產生。', true, true, true, '預約一對一諮詢', '啟用'
  from projects p
 where p.code = 'LO'
on conflict (tenant_slug, project_code, version_code) do update
set title = excluded.title,
...
with q as (
  ...
  select p.tenant_slug, p.code, 'LITE', seed.stage_key, seed.question_no, seed.title, seed.help_text, seed.type, true, seed.sort_order, '啟用'
    from projects p
   cross join (values
...
select q.tenant_slug, q.id, seed.option_key, seed.label, seed.score_weights::jsonb, seed.sort_order, '啟用'
  from q
  join (values
    (1, 'LITE', '我還不太確定，只是想先了解', '{"clarity":1,"urgency":1,"readiness":1}', 10),
    (1, 'FULL', '我有明確問題，但還不知道怎麼處理', '{"clarity":3,"urgency":2,"readiness":3}', 20),
    (1, 'C', '我已經想處理，正在找適合方案', '{"clarity":5,"urgency":4,"readiness":4}', 30),
    (2, 'LITE', '剛開始出現', '{"clarity":1,"urgency":1,"readiness":1}', 10),
    (2, 'FULL', '持續一段時間了', '{"clarity":2,"urgency":3,"readiness":2}', 20),
    (2, 'C', '已經影響到我的安排或決策', '{"clarity":3,"urgency":5,"readiness":3}', 30),
    (3, 'LITE', '釐清問題', '{"clarity":2,"urgency":1,"readiness":2}', 10),
    (3, 'FULL', '知道下一步怎麼做', '{"clarity":3,"urgency":2,"readiness":3}', 20),
...
select p.tenant_slug, p.code, 'LITE', seed.block_key, seed.block_type, seed.title, seed.content::jsonb, seed.sort_order, true, '啟用'
  from projects p
 cross join (values
