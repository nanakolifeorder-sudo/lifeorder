...
async function getQuizAccessCodeData(payload, tenant, user) {
  ...
  const [projects, codes] = await Promise.all([
    ...
    safeQuery(
      `select c.*,
              ...
              count(distinct u.client_email_normalized)::int as unique_email_count,
  ...
  return {
    ...
    codes: codes.rows.map(row => ({
      ...
      uniqueEmailCount: Number(row.unique_email_count || 0),
      perEmailLimit: Number(row.per_email_limit || 1),
...
async function saveQuizAccessCode(payload, tenant, user) {
  ...
  const sql = rowId
    ...
    : `insert into quiz_access_codes(
         ...
         max_uses, per_email_limit, starts_at, ends_at, status, notes
       ...
       set name = excluded.name,
           ...
           per_email_limit = excluded.per_email_limit,
