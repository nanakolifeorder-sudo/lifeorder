...
async function getConsultantAppointments(payload, tenant, user) {
  ...
  if (leadEmails.length) {
    const latestQuizRows = await query(
      ...
      [tenant, payload.projectCode && payload.projectCode !== 'ALL' ? normalizeProject(payload.projectCode) : (leads.rows[0]?.project_code || 'P01'), leadEmails]
    );
...
async function importCRMContacts(payload, tenant, user) {
  ...
  const defaultProjectCode = normalizeProject(payload.projectCode || 'P01');
...
async function saveQuizAccessCode(payload, tenant, user) {
  ...
  const projectCode = normalizeProject(payload.projectCode || 'P01');
  const versionCode = String(payload.versionCode || payload.version || '').trim().toUpperCase();
