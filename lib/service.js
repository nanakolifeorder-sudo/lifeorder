...
async function getAvailableTimes(payload, tenant) {
  ...
  for (const consultant of eligible) {
    ...
    if (provider === 'zoom' && !useInternalTestCalendar) continue;
...
async function submitBooking(payload, tenant) {
  ...
  const allowInternalTestBooking = isInternalTestBookingTenant(tenant, payload, projectCode);
  const useInternalTestCalendar = shouldUseInternalTestCalendar(tenant, consultant, provider, allowInternalTestBooking);
  if (provider === 'zoom' && !useInternalTestCalendar) throw new Error('Zoom OAuth 尚未完成串接，請改用 Google Meet 或 Microsoft Teams。');
