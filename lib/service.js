...
function renderEmailText(templateText, data) {
  ...
  return String(templateText || '').replace(/\{\{\s*(name|email|phone|project|time|meetLink|consultant)\s*\}\}/g, (_match, key) => values[key] || '');
}
  ...
  const result = await query(
    `insert into leads(
     ...
     set project_name = excluded.project_name,
         ...
         status = case
           when leads.status = 'booked' and excluded.status = 'pending' then leads.status
...
async function getSenderConsultant(tenant, projectCode, appointmentId = null) {
  if (appointmentId) {
    const result = await query(
      `select c.*
         from appointments a
         join consultants c on c.tenant_slug = a.tenant_slug and c.id = a.consultant_id
        where a.tenant_slug = $1 and a.id = $2 and c.google_refresh_token is not null
        limit 1`,
      [tenant, appointmentId]
    );
  ...
  const result = await query(
    `select *
      ...
      order by case when permissions = 'ALL' then 0 else 1 end, id asc
      limit 1`,
    [tenant]
  );
  return result.rows[0];
