...
async function submitBooking(payload, tenant) {
  const rescheduleAppointment = payload.rescheduleToken
    ...
    : null;
  const projectCode = normalizeProject(rescheduleAppointment?.project_code || payload.projectCode);
  const project = await projectByCode(tenant, projectCode);
  ...
  const clientName = rescheduleAppointment?.client_name || payload.name;
  const clientEmail = rescheduleAppointment?.client_email || payload.email;
  const clientPhone = rescheduleAppointment?.client_phone || payload.phone || '';
  const answers = rescheduleAppointment?.answers || payload.answers || '';
...
function renderEmailText(templateText, data, options = {}) {
  ...
  return String(templateText || '').replace(
    /\{\{\s*(name|email|phone|project|time|meetLink|consultant|rescheduleLink|客戶姓名|姓名|客戶Email|Email|客戶電話|電話|專案名稱|預約時間|會議連結|顧問姓名|顧問|改期專屬連結)\s*\}\}/g,
    (_match, key) => {
  ...
  for (const template of templates.rows) {
    ...
    await query(
      ...
      [
        ...
        renderEmailText(template.body, data, { html: true }),
  ...
  if (payload.rowId || payload.id) {
    await query(
      ...
      [
        ...
        payload.subject || '',
        sanitizeEmailHtml(payload.body || ''),
        status,
        payload.senderName || '',
...
async function handleAppointmentAction(payload, tenant, user) {
  ...
  if (row.calendar_provider === 'microsoft' && row.microsoft_refresh_token && row.event_id) {
    await deleteMicrosoftCalendarEvent({
      encryptedRefreshToken: row.microsoft_refresh_token,
      calendarId: row.calendar_id || 'primary',
      eventId: row.event_id
    });
  } else if (row.google_refresh_token && row.event_id) {
    await deleteCalendarEvent({
...
async function sendSelfRescheduleLink(payload, tenant, user) {
  ...
  const rescheduleLink = `${appUrl()}/booking?tenant=${encodeURIComponent(tenant)}&p=${encodeURIComponent(appointment.project_code)}&rid=${encodeURIComponent(token)}`;
...
async function saveTenantAsOwner(payload, tenant, user) {
  ...
  const userRow = await transaction(async client => {
    ...
    const inserted = await client.query(
      `insert into tenants(slug, name, owner_name, owner_email, owner_password_hash, app_base_url, created_at)
       select $2, $3, $4, $5, owner_password_hash, app_base_url, created_at
         from tenants
        where slug = $1
       returning slug, name, owner_name, owner_email`,
