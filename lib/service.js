...
function isInternalTestBookingTenant(tenant) {
  return String(tenant || '').toLowerCase() === 'dm-test';
}

function shouldUseInternalTestCalendar(tenant, consultant, provider) {
  if (!isInternalTestBookingTenant(tenant)) return false;
...
async function getAvailableTimes(payload, tenant) {
  ...
  const allowInternalTestBooking = isInternalTestBookingTenant(tenant);
  ...
  for (const consultant of eligible) {
    ...
    const useInternalTestCalendar = shouldUseInternalTestCalendar(tenant, consultant, provider);
