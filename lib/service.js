...
const { isZoomConfigured, createZoomMeeting, deleteZoomMeeting } = require('./zoom');
const {
  zonedDateString,
  zonedTimeToUtc,
  addDaysToDateString,
...
async function getAdminConsultantData(_payload, tenant) {
  const [consultants, projects, tenantSettings] = await Promise.all([
    ...
    query(`select zoom_account_id, zoom_client_id, zoom_client_secret from tenants where slug = $1 limit 1`, [tenant])
  ]);
  return {
    success: true,
...
async function getAvailableTimes(payload, tenant) {
  ...
  const consultants = await query(
    `select * from consultants
      where tenant_slug = $1
        and accepting = true
        and ($2::boolean = true or google_refresh_token is not null or microsoft_refresh_token is not null)`,
  ...
  const eligible = consultants.rows.filter(c =>
    ...
    (consultantCalendarProvider(c) !== 'zoom' || allowInternalTestBooking || (isZoomConfigured(zoomSettings) && Boolean(c.google_refresh_token)))
  );
  ...
  for (const consultant of eligible) {
    ...
    if (provider === 'zoom' && !useInternalTestCalendar && (!isZoomConfigured(zoomSettings) || !consultant.google_refresh_token)) continue;
    ...
    if (!useInternalTestCalendar) {
      try {
        ...
        // Zoom can still run from the system schedule when Google Calendar is temporarily unavailable.
...
async function submitBooking(payload, tenant) {
  ...
  if (provider === 'zoom' && !useInternalTestCalendar && !consultant.google_refresh_token) throw new Error('Zoom 預約仍需要連結 Google Calendar，以檢查既有行程並寄送行事曆邀請。');
