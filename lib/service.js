...
async function sendBookingInviteEmail({ tenant, project, clientEmail, clientName = '', clientPhone = '', answers = '', requireForm = true, waitlistId = null }) {
  ...
  const link = `${appUrl()}/booking?tenant=${encodeURIComponent(tenant)}&p=${encodeURIComponent(project.code)}&invite=${encodeURIComponent(token)}`;
  ...
  const [database, imageUsage, emailUsage, totals] = await Promise.all([
    ...
    query(
      `select
         ...
         count(*) filter (where status = 'failed' and created_at >= date_trunc('month', now()))::int as failed_month
       from email_queue`
    ),
    query(
      `select
