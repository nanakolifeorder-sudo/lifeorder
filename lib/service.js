...
async function submitBooking(payload, tenant) {
  ...
  if (rescheduleAppointment) {
    try {
      if (rescheduleAppointment.old_google_refresh_token && rescheduleAppointment.event_id) {
    ...
    appointmentResult = await query(
      `update appointments
          set project_code = $3, project_name = $4, consultant_id = $5,
              consultant_name = $6, calendar_id = $7, event_id = $8,
              meet_link = $9, start_at = $10, end_at = $11, status = '待開會',
              notes = concat_ws(E'\n', nullif(notes, ''), $12)
        where tenant_slug = $1 and id = $2
        returning id`,
      [
...
async function saveTenantAsOwner(payload, tenant, user) {
  ...
  const result = await query(
    ...
    [
      targetTenant,
      String(payload.tenantName || '').trim() || targetTenant,
      String(payload.ownerName || '').trim(),
      normalizeEmail(payload.ownerEmail)
    ]
  );
  if (!result.rows[0]) throw new Error('找不到這個使用者。');
