...
async function getConsultantAppointments(payload, tenant, user) {
  ...
  if (leadEmails.length) {
    const latestQuizRows = await query(
      `select distinct on (client_email_normalized) id, client_email_normalized
         from quiz_responses
        where tenant_slug = $1 and project_code = $2 and client_email_normalized = any($3::text[])
        order by client_email_normalized, submitted_at desc, id desc`,
    ...
    latestQuizRows.rows.forEach(row => latestQuizByEmail.set(row.client_email_normalized, Number(row.id)));
  ...
  let leadRows = leads.rows.map(row => ({
    ...
    clientEmail: row.client_email,
    clientPhone: row.client_phone,
    answers: leadAnswerPreview.get(latestQuizByEmail.get(String(row.client_email || '').toLowerCase()) || Number(row.latest_quiz_response_id || 0))?.join('\n\n') || row.answers,
    status: row.lifecycle_stage || row.status || 'lead',
    crmContactId: row.crm_contact_id || null,
    quizResponseId: latestQuizByEmail.get(String(row.client_email || '').toLowerCase()) || row.latest_quiz_response_id || null,
    consultant: '',
    meetLink: ''
  }));

  const contactTags = await safeQuery(
    `select id, project_code, name, email, email_normalized, phone, tags, notes, lifecycle_stage, latest_quiz_response_id, purchase_plan, purchase_date
       from crm_contacts
      where tenant_slug = $1${clientProjectClause}
        and coalesce(status, '') not in ('deleted', '刪除')`,
    clientProjectParams
  ...
  const [projects, rows, stats, tagRows] = await Promise.all([
    ...
    query(
      `select r.id, r.public_id, r.project_code, coalesce(p.name, r.project_code) as project_name,
              ...
              c.tags, c.lifecycle_stage, c.notes, c.purchase_plan, c.purchase_date,
...
async function saveCRMContactLabels(payload, tenant, user) {
  ...
  const updated = await query(
    `update crm_contacts
        set tags = $3::text[], notes = $4, lifecycle_stage = $5, purchase_plan = $6, purchase_date = $7, updated_at = now()
      where tenant_slug = $1 and id = $2
      returning id, project_code, name, email, phone, lifecycle_stage, tags, notes, purchase_plan, purchase_date, updated_at`,
    [tenant, contact.id, tags, notes, nextLifecycleStage, purchasePlan, purchaseDate]
  );
  await safeQuery(
    `update leads
        set lifecycle_stage = $4, metadata = metadata || $5::jsonb, updated_at = now()
  ...
  await recordTimelineEvent({
    ...
    metadata: { tags, lifecycleStage: nextLifecycleStage, purchasePlan, purchaseDate }
  });
