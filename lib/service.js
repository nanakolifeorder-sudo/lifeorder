...
async function getConsultantAppointments(payload, tenant, user) {
  ...
  const contactTags = await safeQuery(
    `select id, project_code, name, email, email_normalized, phone, tags, notes, lifecycle_stage, latest_quiz_response_id
       from crm_contacts
      where tenant_slug = $1${clientProjectClause}
        and coalesce(status, '') not in ('deleted', '刪除')`,
    clientProjectParams
  ...
  const [projects, rows, stats, tagRows] = await Promise.all([
    ...
    query(
      `select r.id, r.public_id, r.project_code, coalesce(p.name, r.project_code) as project_name,
              ...
              c.tags, c.lifecycle_stage, c.notes,
...
async function saveCRMContactLabels(payload, tenant, user) {
  ...
  const updated = await query(
    `update crm_contacts
        set tags = $3::text[], notes = $4, lifecycle_stage = $5, updated_at = now()
      where tenant_slug = $1 and id = $2
      returning id, project_code, name, email, phone, lifecycle_stage, tags, notes, updated_at`,
    [tenant, contact.id, tags, notes, nextLifecycleStage]
  );
  await safeQuery(
    `update leads
        set lifecycle_stage = $4, metadata = metadata || $5::jsonb, updated_at = now()
  ...
  await recordTimelineEvent({
    ...
    metadata: { tags, lifecycleStage: nextLifecycleStage }
  });
