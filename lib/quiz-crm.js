...
const LEGACY_LIFEORDER_EXPERIENCE_CLASS_URL = 'https://forms.gle/y1K2j54z9t2DxuGi9';
const CURRENT_LIFEORDER_EXPERIENCE_CLASS_URL = 'https://forms.gle/ct9ydZmcZnJaDEuQ8';
let lifeOrderExperienceClassUrlMigration;
const LIFE_ORDER_EXPERIENCE_CLASS_CTA_VERSION = '2026-09-17-cta-copy-v2';
const LIFE_ORDER_EXPERIENCE_CLASS_CTA_TITLE = '用 90 分鐘，把報告裡最卡的地方理出下一步';
const LIFE_ORDER_EXPERIENCE_CLASS_CTA_CONTENT = {
  label: '我要報名人生秩序體驗課',
  description: `你已經完成完整版測驗，也看見自己目前的人生秩序狀態。

你不需要一次把所有事情整理好。這 90 分鐘，會陪你找出目前最影響生活的一個卡點，整理成可以開始的下一步。

這堂線上人生秩序體驗課，娜娜子會陪你一起回看個人報告，找出此刻最值得優先處理的方向。

你會帶走：

- 看懂哪些地方正在影響你的生活節奏
- 找出目前最該優先整理的一到三個卡點
- 把一直拖著、說不清楚的事，整理成可行的行動方向
- 留下一份更清楚的安排，讓自己和重要的人都更好往下走

**形式｜** 線上 Zoom，報名確認後會以 Email 寄送連結
**時間｜** 90 分鐘
**費用說明｜** 這堂體驗課已包含在完整版測驗費用中，無須額外付費；可報名的場次與時間，請於表單內查看。`,
  openInNewWindow: true
};
let lifeOrderExperienceClassCtaMigration;
const LIFE_ORDER_FAMILY_CULTURE_COPY_VERSION = '2026-09-17';
let lifeOrderFamilyCultureMigration;

function ensureLifeOrderFamilyCultureCopy() {
  if (!lifeOrderFamilyCultureMigration) {
    lifeOrderFamilyCultureMigration = transaction(async client => {
      const settingsResult = await client.query(
        `select coalesce(settings, '{}'::jsonb) as settings
           from quiz_result_settings
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
          limit 1`
      );
      if (settingsResult.rows[0]?.settings?.familyCultureCopyVersion === LIFE_ORDER_FAMILY_CULTURE_COPY_VERSION) return;

      await client.query(
        `update quiz_score_dimensions
            set description = $1, updated_at = now()
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL' and dimension_key = 'family_culture'`,
        ['檢視教育方式、家庭價值、共同規則、角色分工與生活習慣是否被說清楚並持續實踐。']
      );
      const ranges = [
        ['low', '目前最需要注意', '家庭常被放到忙完以後', '工作、責任和臨時狀況一多，家庭很容易只剩下需要處理的事。', `接送、回訊息、付費、照顧都在進行；一家人想怎麼生活、哪些事值得優先留給彼此，卻很少有時間好好談。

孩子遇到問題怎麼陪、家務和照顧怎麼分、金錢怎麼安排、衝突怎麼面對，常在事情發生後才各自處理。家庭裡每個人都在努力，卻少了一套能一起依循的方式。

先保留一個最常被忙碌擠掉的家庭時刻，從一次固定吃飯、每週對話或一條共同規則開始。`],
        ['mid', '已有基礎，下一步要讓彼此更有共識', '家庭有默契，還需要讓共識更穩', '你們已有自己的相處方式，也知道彼此在意什麼。', `工作、照顧、金錢與不同角色同時靠近時，原本的默契容易被日常節奏沖淡。

孩子犯錯怎麼陪、誰負責哪些事、重要開支怎麼談、哪些時間留給家人，仍需要反覆協調。當共同原則沒有被說清楚，最能扛的人往往會承擔更多判斷與提醒。

挑一項最常重複協調的事，整理成每個人都聽得懂、做得到的約定。`],
        ['high', '狀態清楚，適合整理成可延續的家庭共識', '家庭已被放進共同的人生安排', '你們的家庭責任、生活方式與重要價值已有清楚輪廓。', `家人知道這個家重視什麼、遇到事情怎麼談、誰負責什麼，也有一些值得保留的相處習慣。

即使工作很忙、家庭角色持續改變，大家仍有時間回來確認彼此的需求與安排。這份共識讓家庭生活更有方向，也讓下一代能理解自己是在什麼樣的價值裡長大。

隨著孩子、長輩與工作階段改變，定期更新分工、規則與相處方式，讓家庭共識持續貼近現在的生活。`]
      ];
      for (const [rangeKey, label, title, subtitle, body] of ranges) {
        await client.query(
          `update quiz_score_ranges
              set label = $1, title = $2, subtitle = $3, body = $4, updated_at = now()
            where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
              and dimension_key = 'family_culture' and range_key = $5`,
          [label, title, subtitle, body, rangeKey]
        );
      }
      const ageContents = [
        ['under_35', `這個階段不需要急著複製原生家庭，也不必急著定下未來所有規則。比較重要的是，開始看見哪些相處方式想保留，哪些想換一種做法。

放到「家文化」來看，重點是：當你與伴侶、家人或未來的孩子一起生活時，希望大家怎麼說話、怎麼分工、怎麼面對犯錯與做決定。`, '先寫下一句你想帶進自己家庭的原則，例如：有問題先說，不用靠猜。'],
        ['35_45', `這個階段常同時面對工作、伴侶、孩子與原生家庭。最容易累的往往不是事情多，而是每個人都以為你知道該怎麼做。

放到「家文化」來看，重點是把教養、家務、照顧、金錢與相處時間的期待說清楚，讓家庭不是只靠最能扛的人撐住。`, '找一件最近常重複協調的事，和家人訂出一個大家做得到的分工或規則。'],
        ['45_55', `這個階段常要同時照顧下一代、支持長輩，也重新安排自己的生活。若家庭裡沒有共同原則，很多決定很容易全落在少數人身上。

放到「家文化」來看，重點是一起談清楚：健康、金錢、照顧、教育與時間，這個家現在最重視什麼，又該怎麼分工。`, '安排一次家庭對話，先談一件最需要共識的事，而不是一次處理所有問題。'],
        ['55_65', `這個階段常伴隨角色轉換：孩子更獨立、長輩需要更多照顧，自己也開始思考往後想怎麼生活。

放到「家文化」來看，重點是讓家人知道彼此的期待與界線。家庭規則可以調整，但重要價值、責任與照顧方式要有地方能說清楚。`, '把一項新的家庭角色或照顧安排談清楚：誰負責、需要什麼協助、遇到變化找誰商量。'],
        ['65_75', `這個階段適合把一路累積的經驗與價值，從「我心裡知道」變成家人能一起使用的共識。

放到「家文化」來看，不只留下故事，也留下你怎麼看責任、健康、金錢、關係與孩子成長；這些能幫家人理解你，也讓下一代有可參考的方向。`, '選一個家人都會碰到的主題，分享你的經驗與期待，並聽聽下一代怎麼想。'],
        ['75_plus', `這個階段的家文化，不需要變成一份厚重的家訓。最重要的是讓家人知道：你珍惜什麼、希望大家怎麼互相照顧，以及哪些生活習慣想繼續留著。

這些話若能被說出來，家人以後遇到選擇時，就不只是在猜你的心意。`, '留下一段簡短的話：這個家最希望大家一直記得、一直做到的是什麼。']
      ];
      for (const [ageGroup, body, actionText] of ageContents) {
        await client.query(
          `update quiz_age_dimension_contents
              set body = $1, action_text = $2, updated_at = now()
            where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
              and dimension_key = 'family_culture' and age_group = $3`,
          [body, actionText, ageGroup]
        );
      }
      await client.query(
        `update quiz_result_settings
            set settings = coalesce(settings, '{}'::jsonb) || $1::jsonb, updated_at = now()
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'`,
        [JSON.stringify({ familyCultureCopyVersion: LIFE_ORDER_FAMILY_CULTURE_COPY_VERSION })]
      );
    }).catch(error => {
      lifeOrderFamilyCultureMigration = null;
      throw error;
    });
  }
  return lifeOrderFamilyCultureMigration;
}

// The CTA block keeps its own custom URL, separate from result settings. Repair
// old blocks and immutable report snapshots together so every full report agrees.
function ensureLifeOrderExperienceClassUrl() {
  if (!lifeOrderExperienceClassUrlMigration) {
    lifeOrderExperienceClassUrlMigration = Promise.all([
      query(
        `update quiz_report_blocks
          ...
          where tenant_slug = 'lifeorder'
            ...
            and block_type = 'booking_cta'
            and content::text like '%' || $1 || '%'`,
        [LEGACY_LIFEORDER_EXPERIENCE_CLASS_URL, CURRENT_LIFEORDER_EXPERIENCE_CLASS_URL]
      ),
      query(
        `update quiz_responses
            set report_snapshot = replace(report_snapshot::text, $1, $2)::jsonb
          where tenant_slug = 'lifeorder'
            and project_code = 'LO'
            and version_code = 'FULL'
            and report_snapshot::text like '%' || $1 || '%'`,
        [LEGACY_LIFEORDER_EXPERIENCE_CLASS_URL, CURRENT_LIFEORDER_EXPERIENCE_CLASS_URL]
      )
    ]).catch(error => {
      lifeOrderExperienceClassUrlMigration = null;
      throw error;
    });
  }
  return lifeOrderExperienceClassUrlMigration;
}

// Keep the approved experience-class CTA identical in the editor and existing snapshots.
function ensureLifeOrderExperienceClassCta() {
  if (!lifeOrderExperienceClassCtaMigration) {
    lifeOrderExperienceClassCtaMigration = transaction(async client => {
      const settingsResult = await client.query(
        `select coalesce(settings, '{}'::jsonb) as settings
           from quiz_result_settings
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
      ...
      if (settingsResult.rows[0]?.settings?.experienceClassCtaVersion === LIFE_ORDER_EXPERIENCE_CLASS_CTA_VERSION) return;

      const patch = JSON.stringify(LIFE_ORDER_EXPERIENCE_CLASS_CTA_CONTENT);
      await client.query(
        `update quiz_report_blocks
            set title = $2,
                content = coalesce(content, '{}'::jsonb) || $1::jsonb,
                updated_at = now()
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
            and block_type = 'booking_cta'`,
        [patch, LIFE_ORDER_EXPERIENCE_CLASS_CTA_TITLE]
      );
      await client.query(
        `update quiz_responses
            set report_snapshot = jsonb_set(
              report_snapshot,
              '{blocks}',
              coalesce((
                select jsonb_agg(
                  case when block ->> 'blockType' = 'booking_cta'
                    then jsonb_set(
                      jsonb_set(block, '{title}', to_jsonb($2::text), true),
                      '{content}', coalesce(block -> 'content', '{}'::jsonb) || $1::jsonb, true
                    )
                    else block
                  end
                )
                from jsonb_array_elements(
                  case when jsonb_typeof(report_snapshot -> 'blocks') = 'array'
                    then report_snapshot -> 'blocks'
                    else '[]'::jsonb
                  end
                ) as block
              ), '[]'::jsonb),
          ...
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'
            and jsonb_typeof(report_snapshot -> 'blocks') = 'array'`,
        [patch, LIFE_ORDER_EXPERIENCE_CLASS_CTA_TITLE]
      );
      await client.query(
        `update quiz_result_settings
            set settings = coalesce(settings, '{}'::jsonb) || $1::jsonb, updated_at = now()
          where tenant_slug = 'lifeorder' and project_code = 'LO' and version_code = 'FULL'`,
        [JSON.stringify({ experienceClassCtaVersion: LIFE_ORDER_EXPERIENCE_CLASS_CTA_VERSION })]
      );
    }).catch(error => {
      lifeOrderExperienceClassCtaMigration = null;
      throw error;
    });
  }
  return lifeOrderExperienceClassCtaMigration;
}
...
function normalizeQuizVersion(value, fallback = 'LITE') {
  const base = fallback === undefined || fallback === null ? 'LITE' : String(fallback).trim().toUpperCase();
  const code = String(value || base || 'LITE').trim().toUpperCase() || base || 'LITE';
...
function isEmailLimitedQuizVersion(versionCode, version) {
  const code = normalizeQuizVersion(versionCode || '', '');
  const text = [code, version?.name, version?.description]
    .map(value => String(value || '').toLowerCase())
    .join(' ');
  if (['A', 'LITE', 'FREE', 'TRIAL', 'SIMPLE'].includes(code)) return false;
  if (/(免費|簡易|free|trial|simple)/i.test(text)) return false;
  return /(980|付費|完整版|完整|paid|full|premium|pro)/i.test(text) || ['B', 'FULL', 'PAID', 'PRO', 'PREMIUM'].includes(code);
}
function isFullQuizVersion(versionCode, version) {
  return isEmailLimitedQuizVersion(versionCode, version);
}

function quizReportAccessSecret() {
  if (!process.env.JWT_SECRET) throw new Error('JWT_SECRET is not set.');
  return process.env.JWT_SECRET;
}

function fullReportRequiresVerification(tenant, row) {
  return tenant === 'lifeorder' && isFullQuizVersion(row?.version_code, { version_code: row?.version_code });
}

function createQuizReportAccessToken(tenant, resultId) {
  return jwt.sign({ type: 'quiz_report_access', tenant, resultId: String(resultId) }, quizReportAccessSecret(), { expiresIn: '7d' });
}

function hasQuizReportAccessToken(token, tenant, resultId) {
  try {
    const decoded = jwt.verify(String(token || ''), quizReportAccessSecret());
    return decoded?.type === 'quiz_report_access' && decoded.tenant === tenant && String(decoded.resultId) === String(resultId);
  } catch (_error) {
    return false;
  }
}
...
function extractDemographics(answerRows, rawAnswers = {}) {
  const result = { age: "", ageGroup: "", ageGroupLabel: "", identity: "", hasPets: null };
  if (rawAnswers && typeof rawAnswers === "object") {
    result.age = String(rawAnswers._profile_age || rawAnswers.age || rawAnswers.age_group || "").trim();
    result.ageGroup = normalizeAgeGroup(result.age);
    result.ageGroupLabel = ageGroupLabel(result.ageGroup);
    result.identity = String(rawAnswers._profile_occupation || rawAnswers.occupation || rawAnswers.identity || "").trim().slice(0, 120);
    if (Object.prototype.hasOwnProperty.call(rawAnswers, '_profile_has_pets')) result.hasPets = rawAnswers._profile_has_pets === true || rawAnswers._profile_has_pets === 'true' || rawAnswers._profile_has_pets === '1';
  }
...
async function resolveQuizAccessCode({ tenant, projectCode, versionCode, code, clientPhone }) {
  ...
  if (!accessCode) throw new Error('重測碼不存在，請確認後再送出。');
  const now = Date.now();
  ...
  const phoneUses = await safeQuery(
    `select count(*)::int as count
       from quiz_access_code_usages
      where tenant_slug = $1
        and access_code_id = $2
        and regexp_replace(coalesce(client_phone, ''), '[^0-9]', '', 'g') = regexp_replace($3, '[^0-9]', '', 'g')`,
    [tenant, accessCode.id, clientPhone]
  );
  if (Number(phoneUses.rows[0]?.count || 0) >= Number(accessCode.per_phone_limit || 1)) throw new Error('這組代碼已經被這個電話號碼使用過。');
  return accessCode;
}
...
function liteReportScoreLevel(score) {
  const value = Number(score || 0);
  if (value >= 60) return '可維持';
  if (value >= 30) return '需要關注';
  return '優先處理';
}

function tieBreakOrderForAge(ageValue) {
  const age = Number(ageValue);
  if (!Number.isFinite(age) || age <= 0) return '';
  if (age >= 56) return 'continuity';
  if (age >= 41) return 'internal';
  return 'external';
}

function buildReportContext(displayScores, displayMaxScores, tieBreakOrder = '', scoreLevelForReport = reportScoreLevel) {
  const keys = Object.keys(displayScores || {}).filter(key => Number.isFinite(Number(displayScores[key])));
  const levels = {};
  keys.forEach(key => { levels[key] = scoreLevelForReport(displayScores[key]); });
  const scoreLevels = keys.map(key => levels[key]).filter(Boolean);
  const sameScoreLevel = scoreLevels.length > 1 && new Set(scoreLevels).size === 1;
  let minScore = null;
  keys.forEach(key => {
  ...
  // Lite reports use age to choose one starting point when all three orders share a score band.
  const mainOrder = (sameScoreLevel && tieBreakOrder && keys.includes(tieBreakOrder) ? tieBreakOrder : '') ||
    (tieBreakOrder && tied.includes(tieBreakOrder) ? tieBreakOrder : '') ||
    REPORT_ORDER_PRIORITY.find(key => tied.includes(key)) || tied[0] || '';
  return {
    levels,
    sameScoreLevel,
    mainOrder,
...
function reportBlockOrderHint(block) {
  const content = block?.content || {};
  const text = [block?.title, content.title, content.heading, content.label, content.text, content.body, content.description].filter(Boolean).join(' ');
  if (/外在秩序/.test(text)) return 'external';
  if (/內在秩序/.test(text)) return 'internal';
  if (/延續秩序/.test(text)) return 'continuity';
  return '';
}
...
async function getQuizConfig(payload, tenant) {
  ...
  const versionCode = normalizeQuizVersion(payload.versionCode || payload.version || project.default_quiz_version_code, 'LITE');
...
async function buildQuizScoreAndAnswers({ tenant, projectCode, versionCode, responseId, rawAnswers }) {
  if (tenant === 'lifeorder' && normalizeProject(projectCode) === 'LO' && normalizeQuizVersion(versionCode) === 'FULL') {
    await Promise.all([ensureLifeOrderExperienceClassUrl(), ensureLifeOrderExperienceClassCta(), ensureLifeOrderFamilyCultureCopy()]);
  }
  ...
  const demographics = extractDemographics(answerRows, rawAnswers);
  ...
  dimensions.rows.forEach(dimension => {
    const hidePetContent = demographics.hasPets === false && dimension.dimension_key === 'pets';
  ...
  const reportContext = buildReportContext(
    displayScores,
    displayMaxScores,
    isFullReport ? '' : tieBreakOrderForAge(demographics.age),
    isFullReport ? reportScoreLevel : liteReportScoreLevel
  );
  const scoreSummary = {
    scores,
    ...
    lite_single_recommendation: !isFullReport && reportContext.sameScoreLevel,
    daily_scenario_copy: '',
    demographics,
  ...
  const shouldShowSingleLiteRecommendation = scoreSummary.lite_single_recommendation;
  const reportMatchedContent = shouldShowSingleLiteRecommendation
    ? matchedContent.filter(item => item.dimensionKey === scoreSummary.main_order)
    : matchedContent;
  const reportBlocks = shouldShowSingleLiteRecommendation
    ? visibleBlocks.filter(block => {
      const orderHint = reportBlockOrderHint(block);
      return !orderHint || orderHint === scoreSummary.main_order;
    })
    : visibleBlocks;
  const reportSnapshot = {
    ...
    matched_content: reportMatchedContent,
    blocks: reportBlocks,
    ...
    const templates = await safeQuery(
      `select * from email_templates
        where tenant_slug = $1 and project_code = $2 and trigger_name = $3 and status not in ('停用', 'disabled', 'deleted')
          and coalesce(nullif(metadata->>'versionCode', ''), $4) = $4
        order by time_param asc, id asc`,
      [tenant, projectCode, triggerName, quizResponse.version_code]
    );
...
async function submitQuiz(payload, tenant) {
  ...
  const rawPhone = payload.phone || payload.clientPhone || '';
  const countryCode = String(payload.phoneCountryCode || payload.clientPhoneCountryCode || '').trim() || (/^0?9\d{8}$/.test(String(rawPhone).replace(/\D/g, '')) ? '+886' : '');
  const clientPhone = normalizePhone(rawPhone, countryCode);
  ...
  if (emailLimited && !isValidInternationalPhone(rawPhone, countryCode)) throw new Error(phoneValidationMessage(countryCode));
  const existingResponse = emailLimited ? await safeQuery(
    `select id, public_id, submitted_at
      ...
      where tenant_slug = $1
        and project_code = $2
        and version_code = $3
        and regexp_replace(coalesce(client_phone, ''), '[^0-9]', '', 'g') = $4
        and status = 'completed'
      order by submitted_at desc, id desc
      limit 1`,
    [tenant, projectCode, versionCode, clientPhone]
  ) : { rows: [] };
  const previousResponse = existingResponse.rows[0] || null;
  const suppliedAccessCode = payload.accessCode || payload.couponCode || payload.retakeCode;
  const accessCode = suppliedAccessCode
    ? await resolveQuizAccessCode({ tenant, projectCode, versionCode, code: suppliedAccessCode, clientPhone })
    : null;
  const entitlement = emailLimited && payload.grant
    ? await authorizeFullQuizSubmission({ tenant, grant: payload.grant, email: clientEmail, phone: rawPhone, phoneCountryCode: countryCode })
    : null;
  if (emailLimited && !entitlement && !accessCode) {
    throw new Error('請使用專屬完整版授權連結，或先輸入有效優惠碼。');
  }
  if (previousResponse && !accessCode) {
    return {
      success: false,
      code: 'QUIZ_ALREADY_COMPLETED',
      message: '這個電話號碼已經完成過本次完整版測驗。若需要再次作答，請輸入重測碼。',
  ...
  if (accessCode?.id) {
    await safeQuery(
      `insert into quiz_access_code_usages(
         ...
         client_email, client_phone, code, usage_type, metadata
       ) values($1,$2,$3,$4,$5,$6,$7,$8,'quiz_retake',$9::jsonb)`,
      [tenant, accessCode.id, projectCode, versionCode, quizResponse.id, clientEmail, clientPhone, accessCode.code, JSON.stringify({ retakeOfResponseId: previousResponse?.id || null })]
    );
    await safeQuery(
  ...
  if (entitlement) {
    await completeFullQuizEntitlement({
      tenant,
      entitlementId: entitlement.id,
      quizResponseId: quizResponse.id,
      quizResultId: quizResponse.public_id
    });
    await recordTimelineEvent({
      tenant,
      projectCode,
      crmContactId: contact?.id || entitlement.crm_contact_id || null,
      leadId: lead.id,
      quizResponseId: quizResponse.id,
      eventType: 'full_quiz_entitlement_completed',
      title: '完整版測驗已完成',
      body: '已完成專屬完整版授權所對應的測驗。',
      metadata: { entitlementId: entitlement.id, resultId: quizResponse.public_id }
    });
  }
  ...
  if (accessCode?.code_type === 'invite') {
    await recordTimelineEvent({
      tenant,
      projectCode,
      crmContactId: contact?.id,
      leadId: lead.id,
      quizResponseId: quizResponse.id,
      eventType: accessCode.batch_id ? 'group_invite_completed' : 'single_use_invite_completed',
      title: accessCode.batch_id ? '團購完整版邀請碼已使用' : '單次完整版邀請碼已使用',
      body: `已透過邀請碼 ${accessCode.code} 完成完整版測驗。`,
      metadata: { accessCode: accessCode.code, accessCodeId: accessCode.id, batchId: accessCode.batch_id || null, resultId: quizResponse.public_id }
    });
  }
  ...
  return {
    ...
    quizResponseId: quizResponse.id,
    reportUrl,
    reportAccessToken: emailLimited ? createQuizReportAccessToken(tenant, quizResponse.public_id) : '',
    scoreSummary,
    reportSnapshot
  };
}

async function verifyFullQuizReportAccess(payload, tenant) {
  const resultId = payload.quizResultId || payload.qrid || payload.resultId || payload.rid;
  if (!resultId) throw new Error('缺少測驗結果 ID。');
  const result = await query(
    `select id, public_id, project_code, crm_contact_id, lead_id, client_email, client_phone, version_code
       from quiz_responses
  ...
  if (!fullReportRequiresVerification(tenant, row)) return { success: true, reportAccessToken: '' };
  const email = normalizeEmail(payload.email || payload.clientEmail);
  const phone = normalizePhone(payload.phone || payload.clientPhone, payload.phoneCountryCode || payload.clientPhoneCountryCode || '');
  if (!email || !email.includes('@')) throw new Error('請輸入正確的 Email。');
  if (!phone) throw new Error('請選擇國碼並輸入正確的電話號碼。');
  if (email !== normalizeEmail(row.client_email) || phone !== normalizePhone(row.client_phone)) {
    throw new Error('Email 或電話與本份完整版報告資料不符。');
  }
  await recordTimelineEvent({
    tenant,
    ...
    eventType: 'quiz_report_verified',
    title: '已確認完整版報告存取資格'
  });
  return { success: true, reportAccessToken: createQuizReportAccessToken(tenant, row.public_id), expiresInDays: 7 };
}

async function getQuizResult(payload, tenant) {
  const resultId = payload.quizResultId || payload.qrid || payload.resultId || payload.rid;
  if (!resultId) throw new Error('缺少測驗結果 ID。');
  if (tenant === 'lifeorder') await Promise.all([ensureLifeOrderExperienceClassUrl(), ensureLifeOrderExperienceClassCta()]);
  const result = await query(
  ...
  const row = result.rows[0];
  if (!row) return { success: false, message: '找不到測驗結果。' };
  if (fullReportRequiresVerification(tenant, row) && !hasQuizReportAccessToken(payload.reportAccessToken || payload.reportToken, tenant, row.public_id)) {
    return { success: false, code: 'REPORT_AUTH_REQUIRED', message: '請先確認完整版報告的存取資格。', requiresReportVerification: true };
  }
  await recordTimelineEvent({
    tenant,
    projectCode: row.project_code,
...
function crmReportUrl(tenant, projectCode, publicId) {
  if (!publicId) return '';
  return `${appUrl()}/report?tenant=${encodeURIComponent(tenant)}&p=${encodeURIComponent(projectCode || 'LO')}&qrid=${encodeURIComponent(publicId)}`;
}
...
async function getCRMContactDetail(payload, tenant) {
  ...
  if (!payload.viewerTenantWide && Number(contact.assigned_consultant_id || 0) !== Number(payload.viewerConsultantId || 0)) {
    return { success: false, message: '你沒有權限查看這位客戶。' };
  }
  const [quizResponses, quizAnswers, appointments, emails, timeline, payments, fullEntitlements] = await Promise.all([
    safeQuery(
    ...
    safeQuery(
      `select merchant_order_no, trade_no, product_code, product_name, amount,
              payment_type, status, paid_at, created_at
         from payment_orders
        where tenant_slug = $1
          and status = 'paid'
          and (
            (nullif($2, '') is not null and lower(client_email) = lower($2))
            or (
        ...
        limit 50`,
      [tenant, contact.email || '', contact.phone || '']
    ),
    listFullQuizEntitlements({ tenant, crmContactId: contact.id })
  ]);
  ...
  const paymentHistory = payments.rows.map(row => ({
  ...
  const crmSummary = {
    ...
    totalPaidAmount: paymentHistory.reduce((total, item) => total + Number(item.amount || 0), 0),
    ...
    latestPaymentAt: latestPayment?.paidAt || latestPayment?.createdAt || null,
...
async function validateFullQuizAccessCode(payload, tenant) {
  const projectCode = normalizeProject(payload.projectCode || 'LO');
  const versionCode = normalizeQuizVersion(payload.versionCode || payload.version || 'FULL', 'FULL');
  if (!isFullQuizVersion(versionCode, { version_code: versionCode })) throw new Error('這不是完整版測驗優惠碼。');
  const accessCode = await resolveQuizAccessCode({
    tenant,
    projectCode,
    versionCode,
    code: payload.accessCode || payload.code || '',
    clientPhone: ''
  });
  return { success: true, code: accessCode.code, expiresAt: accessCode.ends_at || null, remainingUses: accessCode.max_uses === null ? null : Math.max(0, Number(accessCode.max_uses) - Number(accessCode.used_count || 0)) };
}
function normalizeReportVersion(value) {
  return normalizeQuizVersion(value, 'LITE');
}
...
function normalizeReportBlockContent(type, content) {
  ...
  if (type === 'booking_cta') return {
    ...
    appendParams: raw.appendParams === true,
    appendFields: Array.isArray(raw.appendFields) ? raw.appendFields.filter(item => ['tenant', 'p', 'qrid', 'name', 'email'].includes(item)).slice(0, 5) : [],
    openInNewWindow: raw.openInNewWindow === true
  };
...
function defaultQuizReportTitle(versionCode) {
  const code = normalizeQuizVersion(versionCode || 'LITE', 'LITE');
  if (code === 'FULL') return '完整版測驗報告';
  if (code === 'LITE') return '免費版測驗報告';
  return '你的檢測報告';
}
...
async function getQuizReportEditor(payload, tenant) {
  ...
  const defaultSettings = {
    title: defaultQuizReportTitle(versionCode),
  ...
  return transaction(async client => {
    await client.query(
      ...
      [
        ...
        projectCode,
        versionCode,
        String(settings.title || defaultQuizReportTitle(versionCode)).slice(0, 240),
