...
async function submitQuiz(payload, tenant) {
  ...
  const clientPhone = normalizePhone(payload.phone || payload.clientPhone || '');
  ...
  if (emailLimited && !/^09[0-9]{8}$/.test(clientPhone)) throw new Error('完整版測驗請輸入 09 開頭的 10 碼台灣手機號碼。');
