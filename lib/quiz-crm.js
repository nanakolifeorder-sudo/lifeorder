...
function buildReportContext(displayScores, displayMaxScores, tieBreakOrder = '') {
...
async function buildQuizScoreAndAnswers({ tenant, projectCode, versionCode, responseId, rawAnswers }) {
  ...
  const reportContext = buildReportContext(displayScores, displayMaxScores, isFullReport ? '' : tieBreakOrderForAge(demographics.age));
  const scoreSummary = {
    scores,
